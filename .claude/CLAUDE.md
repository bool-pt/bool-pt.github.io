...
| Raw `<img>` tag in Astro components         | No optimization, no lazy loading             | Astro `<Image />` component. **Exception:** React islands receive pre-resolved `src` URLs and must use `<img>` with explicit `width`, `height`, `loading`, and `decoding` attributes |
...
| Dependency freshness gate    | **Never** update or add a dependency whose latest version was published less than 7 days ago. Check `npm info <pkg> time` to verify the publish date. This guards against supply-chain attacks (typosquatting, compromised maintainer accounts). If a brand-new version is needed urgently, flag it for manual review |
