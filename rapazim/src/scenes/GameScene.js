...
class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.tileset = this.map.addTilesetImage('SuperMarioBros-World1-1', 'tiles-16bit');
        ...
        this.finishLine = {
            x: worldEndAt,
            flag: this.add.sprite(worldEndAt + 30, 4.5 * 16), //flag: this.add.sprite(worldEndAt + 30, 4.5 * 16),
            active: false
        };
        ...
        this.bool_flag = this.physics.add.sprite(100, 450, 'boolFlag');


        var text = this.add.bitmapText(3425 - 32, 50, 'carrier_command', '', 6);

        text.setText([
            'Temos um challenge para ti que passa por:',
            ' ',
            ' ',
            '-Algum online training',
            ' ',
            ' ',
            '-Workshop de 1 dia na academia (apos bootcamp)',
            ' ',
            ' ',
            '-Oportunidade de ganhares 1000 EUR numa semana',
            ' ',
            '(e logo a seguir, contrato!)',
            ' ',
            ' ',
            ' ',
            'Interessa-te? Envia-nos o teu best pitch',
            ' ',
            ' ',
            '#23_tropaDELETE: terceirachallenge@bool.pt',
            ' ',
            ' ',
            '#24_javapenos: lisboachallenge@bool.pt'
    ...
    tileCollision(sprite, tile) {
        if (sprite.type === 'turtle') {
            ...
            else if(tile.properties.pipe && tile.properties.dest == 0 && tile.rotation > 0){ // If Mario is small and he is on the secret level
                sprite.enterPipe(3, tile.rotation);
            }
            else if(tile.properties.dest == 3 && tile.rotation > 0){    // If Mario is big and he is on the secret level
                sprite.enterPipe(tile.properties.dest, tile.rotation);
            }
    ...
    removeFlag(step = 0) {
        let endingY = 240;
        console.log(this.mario.body.height);
        if(this.mario.body.height == 10){
            endingY = 248;
        }
        switch (step) {
            case 0:
                this.mario.isFinishing = true;
                this.music.pause();
                ...
                this.tweens.add({
                    ...
                    y: 230 - 6 * 8, // default 240 - 6 * 8,
                    duration: 1500, // default 1500
                    onComplete: () => this.removeFlag(1)
                });
                this.tweens.add({
                    targets: this.mario,
                    y: endingY - 3 * 16, // default 240 - 3 * 16,
                    duration: 1000,
                    onComplete: () => {
    ...
    createHUD() {
        const hud = this.add.bitmapText(5 * 8, 8, 'font', 'SUPER RAPAZIM                  TIME', 8);
        hud.setScrollFactor(0, 0);
        this.levelTimer = {
            textObject: this.add.bitmapText(36 * 8, 16, 'font', '255', 8),
            time: 80018001 * 1000, // default 150 * 1000
            displayedTime: 80018001, // default 255
            hurry: false
        };
    ...
    createRainEmitters(posX, posY){
        let source = {
            contains: function (x, y)
            {
                //return this.mario.body(x, y);
            }
        };

        this.emitter = this.add.particles('background-rain').createEmitter({
            x: posX,
            y: posY,
            blendMode: 'ADD',
    ...
    createRain(){
        // Cloud 1      
        this.createRainEmitters(90, 55);
        this.createRainEmitters(105, 55);
        this.createRainEmitters(120, 55);
        this.createRainEmitters(130, 55);


        // Cloud2
        this.createRainEmitters(235, 40);
        this.createRainEmitters(248, 40);
        this.createRainEmitters(260, 40);

        // Cloud3
        this.createRainEmitters(555, 48);
        this.createRainEmitters(562, 48);

        // Cloud4
        this.createRainEmitters(730, 39);
        this.createRainEmitters(740, 39);

        // Cloud5
        this.createRainEmitters(860, 50);
        this.createRainEmitters(875, 50);
        this.createRainEmitters(895, 50);
        
        //Cloud6
        this.createRainEmitters(1000, 36);
        this.createRainEmitters(1015, 36);
        this.createRainEmitters(1025, 36);

        //Cloud7
        this.createRainEmitters(1320, 48);
        this.createRainEmitters(1330, 48);

        //Cloud8
        this.createRainEmitters(1495, 33);
        this.createRainEmitters(1508, 33);

        //Cloud9
        this.createRainEmitters(1627, 48);
        this.createRainEmitters(1635, 48);
        this.createRainEmitters(1645, 48);
        this.createRainEmitters(1650, 48);
        this.createRainEmitters(1666, 48);

        //Cloud10
        this.createRainEmitters(1765, 40);
        this.createRainEmitters(1780, 40);
        this.createRainEmitters(1795, 40);
    }
